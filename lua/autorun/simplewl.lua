...
if SERVER then
AddCSLuaFile("autorun/wl_mgui.lua")
local RplcConf = {
...
if SWhitelist[team.GetName(bt)] then
				local STeam = SWhitelist[team.GetName(bt)]
	if isbool(STeam[ply:SteamID()]) then return end
...
hook.Add("PlayerDisconnected","WL_autoName_Disc",function(ply)
local bt = ply:Team()
if SWhitelist[team.GetName(bt)] then
				local STeam = SWhitelist[team.GetName(bt)]
					PrintTable(STeam)
